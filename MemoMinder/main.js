...
function saveMemo(e) {
  ...
  split_client.track('Memo_added', 0, {priority: memoPriority});
...
function setStatusDone(id) {
  ...
  for (var i = 0; i < memos.length; i++) {
    if (memos[i].id == id) {
      memos[i].status = 'Done';
      split_client.track('Memo_closed', 0, {priority: memos[i].priority});
    }
  }
...
function deleteMemo(id) {
  ...
  for (var i = 0; i < memos.length; i++) {
    if (memos[i].id == id) {
      split_client.track('Memo_deleted', 0, {priority: memos[i].priority});
      memos.splice(i, 1);
    }
  }
