...
function saveMemo(e) {
  ...
  analytics.track("Memo_added", {priority: memoPriority, nestedProperty: [1, 2, {key: 'value'}]});
...
function setStatusDone(id) {
  ...
  for (var i = 0; i < memos.length; i++) {
    if (memos[i].id == id) {
      memos[i].status = 'Done';
      analytics.track("Memo_closed", {priority: memos[i].priority});
    }
  }
...
function deleteMemo(id) {
  ...
  for (var i = 0; i < memos.length; i++) {
    if (memos[i].id == id) {
      analytics.track("Memo_deleted", {priority: memos[i].priority});
      memos.splice(i, 1);
    }
  }
