...
/*  e.preventDefault(); */
  ...
  ga('send',{
        hitType: 'event',
	eventCategory: {'products': [{'name': 'detergent'},{'name': 'cat food'}, {'name': 'CocaCola'}]},
        eventAction: 'ecommerce_event',
        eventLabel: memoPriority
        });
  ...
  for (var i = 0; i < memos.length; i++) {
    if (memos[i].id == id) {
      ...
      ga('send',{
            hitType: 'event',
            eventCategory: 'Memos',
            eventAction: 'Memo_closed',
            eventLabel: memos[i].priority
            });
    }
  }
